...
Mover[] movers = new Mover[5];  //create an array with 5 Mover objects
int value=10;
...
void draw() {
  ...
  for (int i = 0; i < movers.length; i++) {
    ...
    PVector wind = new PVector(0.01, 0);  //Create a new PVector of x=0.01, y=0
    PVector gravity = new PVector(0, 0.1*movers[i].mass);  //Create a new PVector of x=0,y=1/10th of our Mover mass

    float c = 0.05;
    PVector friction = movers[i].velocity.get();  //Get the velocity vector of the current Mover object.
    //make our friction the “opposite” of our current velocity;
    //Then normalize the friction vector; Then scale the vector by 5% (0.05);
    friction.mult(-1); 
    friction.normalize();
    friction.mult(c);
